Shader "Custom/HorizontalGaussianBlur" {
Properties {
	_MainTex ("", 2D) = "White" {}
}
SubShader {
	Pass {
		ZTest Always Cull Off ZWrite Off Fog { Mode off } //Parametrage du shader pour éviter de lire, écrire dans le zbuffer, désactiver le culling et le brouillard sur le polygone
            ...
            float4 frag(Vertex2Pixel i) : COLOR 
            {
				...
				Color += tex2D(_MainTex, float2(i.uv.x - 4.0f * distance, i.uv.y)) * 0.05;
				Color += tex2D(_MainTex, float2(i.uv.x - 3.0f * distance, i.uv.y)) * 0.09;
				Color += tex2D(_MainTex, float2(i.uv.x - 2.0f * distance, i.uv.y)) * 0.12;
				Color += tex2D(_MainTex, float2(i.uv.x - distance, i.uv.y)) * 0.15;
